...
import { AnimatedSection, AnimatedStaggerChildren, AnimatedItem } from "@/components/AnimatedPageWrapper";
        ...
        <AnimatedSection id="projects" className="space-y-8" variant="gentleScale" delay={0.1}>
          {allPosts.length ? (
            <div className="relative -mx-4 px-4">
              <AnimatedStaggerChildren className="flex space-x-6 overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory" staggerDelay={0.05}>
                {allPosts.map((post, index) => (
                  <AnimatedItem key={post.slug} variant="softFadeUp">
                    <Link
